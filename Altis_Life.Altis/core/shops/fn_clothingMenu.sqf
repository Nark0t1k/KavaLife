...
if((SEL(_this,3) isEqualTo "reb" && !license_civ_rebel)) exitWith {hint localize "STR_Shop_NotaReb"; closeDialog 0;};
...
if((life_clothing_purchase select 3) isEqualTo -1) then {
	if(life_oldVest != vest player) then {
		if(life_oldVest isEqualTo "") then {removeVest player;} else {
			player addVest life_oldVest;
			{[_x,true,false,false,true] call life_fnc_handleItem;true;} count life_oldVestItems;
		};
	};
};
