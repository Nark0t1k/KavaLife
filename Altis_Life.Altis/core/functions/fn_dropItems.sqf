...
*/
private["_obj","_unit","_item","_value","_type"];
_unit = _this select 0;
_type = _this select 1;

if(_type)then
{
	_type = (("true" configClasses (missionConfigFile >> "VirtualItems")) + ["life_KavaCash"]);
}else
{
	_type = ("true" configClasses (missionConfigFile >> "VirtualItems"));
};
    ...
    switch (_item) do {
        ...
        case "life_KavaCash": {
            if (life_KavaCash > 0) then {
                ...
                _obj setPos _pos;
                [_obj] remoteExecCall ["life_fnc_simDisable",RANY];
                missionNamespace setVariable ["life_KavaCash",0];
            };
        };
