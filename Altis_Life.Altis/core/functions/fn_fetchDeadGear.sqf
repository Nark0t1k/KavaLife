...
if((side _unit) isEqualTo civilian) then
{
	if(!((headgear _unit) in ["H_CrewHelmetHeli_B","G_Balaclava_TI_blk_F","H_Beret_blk","H_Beret_02","H_MilCap_gen_F","H_Beret_Colonel"]))then
	{
...
if (!(primaryWeapon _unit isEqualTo "")) then {
	if(side _unit isEqualTo civilian)then
	{
		if(!((primaryWeapon _unit) in ["arifle_SPAR_01_blk_F","arifle_SPAR_03_blk_F","arifle_CTAR_blk_F","srifle_DMR_03_khaki_F"]))then
		{
		}else
		{
		};
