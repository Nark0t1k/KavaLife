...
private["_uidVeh","_d","_e","_dialog","_myListbox","_selectedOffer","_id","_seller","_item","_value","_checkid","_cnt","_status","_online","_pid","_unit","_type","_amount","_diff","_uid","_caller","_handle","_tamount","_iCheck"];
...
if (player getVariable["ahID",0] != _id && !_e) exitWith {hint parseText "Someone may be buying or retrieving this item as we speak!";};

if (_seller isEqualTo _uid) exitWith {hint parseText "You cannot purchase your own item idiot!";};

{_checkid = _x select 0;if (_checkid isEqualTo _id) then {_status = _x select 7};true;}count all_ah_items;
if (_status != 0) exitWith {hint parseText "This item has already been sold or expired!";}; //NOT FOR SALE ANYMORE
if (life_KavaBank < _value) exitWith {hint parseText "You don't have enough cash in your bank account!";};
...
switch (_type) do {
	...
	case 2: {
		...
		[player, _seller,_uidVeh] remoteExecCall ["TON_fnc_HV_changeOwner",RSERV];
	};
};
