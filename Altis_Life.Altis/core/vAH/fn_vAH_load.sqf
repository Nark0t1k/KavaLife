...
private["_dialog","_myListbox","_curOffer","_id","_vehicleInfo","_type","_amount","_item","_pic","_itemName","_price","_seller","_sellerName","_status","_time","_search","_allAH","_found"];
	...
	} else {
{
	...
	if (_status isEqualTo 0)then {
		if (_type isEqualTo 2) then {
			_myListbox lbAdd format ["1 lots de %3 pour $%2 par %1, expires dans %4 hours",_sellerName,[_price] call life_fnc_numberText,_itemName,[_time] call life_fnc_numberText];
		}else
		{
			_myListbox lbAdd format ["%5 lots de %3 pour $%2 par %1, expires dans %4 hours",_sellerName,[_price] call life_fnc_numberText,_itemName,[_time] call life_fnc_numberText,[_amount] call life_fnc_numberText];
		};
...
hint parseText "Successfully loaded offers";

((findDisplay 15100) displayCtrl 15160) ctrlSetText "textures\Ekav.jpg";
