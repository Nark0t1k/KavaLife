...
if (_status isEqualTo 3) exitWith {hint parseText "You cannot retrieve an item that is in your inventory already!";};
...
_found = [];
{_checkid = _x select 0;if (_checkid isEqualTo _id) then {_found pushBack _checkid};true;}count all_ah_items;
if (count _found isEqualTo 0) exitWith {hint parseText "This item has been sold recently";};
...
hint parseText "Item Successfully Retrieved";
closedialog 0;