...
[] spawn {
	...
	while {true} do {
		...
		if(!([civilian,getPos player,30] call life_fnc_nearUnits) && (player getVariable ["restrainedciv",FALSE]) && vehicle player isEqualTo player) exitWith {
...
[format[localize "STR_Cop_Retrained",_cop getVariable ["realname",name _cop]]] call life_fnc_showNotification;
