...
switch (true) do
{
	case (player isEqualTo leader(group player)) :
	{
		ctrlShow[2623,true];
	};
	
	case ((getPlayerUID player) isEqualTo _ownerID) :
	{
		ctrlShow[2623,true];
	};
...
for "_i" from 0 to (count _units)-1 do
{
	switch (true) do
	{
		case ((_units select _i) isEqualTo leader(group player)) :
		{
			_members lbAdd format["%1 [Chef]",name (_units select _i)];
		};
		
		case ((getPlayerUID (_units select _i)) isEqualTo _ownerID) :
		{
			_members lbAdd format["%1 [Createur]", name (_units select _i)];
		};
