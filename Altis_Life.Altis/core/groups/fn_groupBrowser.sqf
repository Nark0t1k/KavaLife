...
lbClear _groups;
{
	...
	if(!isNull (_group select 1)) then
	{
		...
		_groups lbAdd format["%1 [Membres: %2]%3",_group select 0,count (units(_group select 1)),_locked];
		_groups lbSetData [(lbSize _groups)-1,_group select 0];
	};
} foreach (life_group_list);

if(((lbSize _groups)-1) isEqualTo -1) then
{
	_groups lbAdd "Aucun groupe pour le moment.";
	ctrlShow[2522,false];
};